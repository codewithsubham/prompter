...
adminsocket.on('connection' , async (socket) => {


    let name = socket.handshake.query.classCode;

    try{
        let result = await Mysql.retriveQuery(name)

        for(let data of result){
            data.retrived = true
            adminsocket.emit(name , data);
        }

    }catch{};
    
    // this are new line of codes only needed to be add
    ...
    socket.on(name , (data) => {
        ...
        //store data for archieve mode
        try{
            Mysql.storeQuery(name , data)
        }catch(e){

        }
        if(!data.hasOwnProperty('retrived')){
        ...
        if(data.hasOwnProperty('self')){
            ...
            if(data.self){
                // remove query when aknowlegde receive
                adminsocket.emit(name , {isConnected:true});
                ...
                Mysql.deleteReceiveQuery(name , data)
            }
        }
